...
| `pnpm cards:sync` | Regenerate the Luau snapshot of `public/super-chess.json` (see [Shared data](#shared-data-cards--engine-constants)) |
| `pnpm trello:lists` | List the open lists (and their IDs) on the Super Chess Trello board (see [Bug reports](#bug-reports--trello)) |
...
The in-game **🐞 bug report** button (top of the play panel) opens a modal
that captures a full game snapshot (FEN, both hands, super-state, recent
events, validation, session debug log). The modal exposes three ways to
get that snapshot off the user's machine:

1. **Send to Trello →** posts a new card to the Super Chess Trello board
   ([4oHJjs1D/super-chess](https://trello.com/b/4oHJjs1D/super-chess)).
2. **Copy report** copies the raw JSON to clipboard.
3. **Download .json** saves it as `super-chess-bug-<timestamp>.json`.

Option 1 only works when the Cloudflare Worker has the right secrets. The
Worker route lives at `POST /api/bug-report` and is implemented in
[`src/worker/index.ts`](./src/worker/index.ts). It calls Trello's REST
API server-side, so the API key + token never touch the client bundle.

### One-time setup

1. **Create the destination list.** On
   [trello.com/b/4oHJjs1D/super-chess](https://trello.com/b/4oHJjs1D/super-chess),
   add a list named e.g. *🐞 Bug Reports* (or reuse an existing one).
2. **Get your Trello credentials.** Visit
   [trello.com/app-key](https://trello.com/app-key) and copy the **API
   key**, then click the *Token* link on that page and authorise to
   copy the **API token**.
3. **Find the list ID** by running:

   ```bash
   TRELLO_API_KEY=… TRELLO_API_TOKEN=… pnpm trello:lists
   ```

   The script prints every open list on the board with its 24-hex-char
   ID. Copy the one for the destination list.
4. **Push secrets to production.** Run these once and paste each value
   when prompted:

   ```bash
   npx wrangler secret put TRELLO_API_KEY
   npx wrangler secret put TRELLO_API_TOKEN
   npx wrangler secret put TRELLO_LIST_ID
   ```

5. **Re-deploy** so the Worker picks up the new secrets:
...
`wrangler dev` reads secrets from a (gitignored) `.dev.vars` file. Copy
the example and fill in your values:

```bash
cp .dev.vars.example .dev.vars
$EDITOR .dev.vars
pnpm cf:dev          # Worker on :8787, hits real Trello with your dev token
```

When using plain `pnpm dev` (the Vite dev server on :5173), the bug-report
button will hit `/api/bug-report` on its own origin and fail with a 404
— that's expected. Run `pnpm cf:dev` instead if you want to test the
full submit flow locally.

### Abuse protection

- The Worker per-IP-rate-limits `/api/bug-report` to **3 requests / minute**
  via the Cloudflare rate-limit binding (`BUG_REPORT_LIMIT` in
  `wrangler.jsonc`; the binding only supports 10s or 60s windows). The
  binding is optional at runtime — if your plan doesn't include it, the
  Worker just doesn't enforce the limit.
- Title is clamped to 120 chars, description to ~14 KiB (Trello's own
  cap is 16 KiB).
- The `/api/bug-report` route is the **only** public POST endpoint;
  everything else falls through to the static SPA via `env.ASSETS.fetch`.

## Shared data (cards + engine constants)

The card definitions, piece values, piece-square tables, and high-level rules
(max hand size, max moves per game) all live in a single canonical JSON file:
**`public/super-chess.json`**. Edit it there — both the web project and the
Roblox port read from it.

```
public/super-chess.json     ← hand-edited single source of truth
        │
        │ (imported directly)               (regenerated by pnpm cards:sync)
        ▼                                   ▼
src/data/superChessData.ts          roblox/SuperChessData.luau
  exports CARD_DEFINITIONS,           module containing the JSON as an
  PIECE_VALUES, PST tables,           embedded string + version stamp
  MAX_HAND_SIZE, …                              │
                                                ▼
                                      Game.ReplicatedStorage.SuperChess
                                        .Modules.SuperChessData
                                                │
                                                ▼
                                      Game.ReplicatedStorage.SuperChess
                                        .Modules.SharedData
                                        (decodes embeddedJson, optionally
                                        re-fetches at boot via HttpService)
```

Workflow:

1. Edit `public/super-chess.json` (e.g. tweak card copies, change a PST, retune
   piece values).
2. The web project picks up the change immediately — Vite hot-reloads the JSON
   import. Run `pnpm test` to make sure the data-shape test still passes.
3. Run `pnpm cards:sync`. This rewrites `roblox/SuperChessData.luau` with the
   updated JSON embedded as a long-bracket string. The file header records the
   regeneration timestamp, card count, and version.
4. Push `roblox/SuperChessData.luau` into Studio at
   `Game.ReplicatedStorage.SuperChess.Modules.SuperChessData` (the Cursor
   session can do this via the Studio MCP `multi_edit` tool, or you can paste
   the file by hand).
5. Optionally set `Modules.SharedData.SOURCE_URL` to your dev URL
   (`http://localhost:5173/super-chess.json`) or production URL
   (`https://super-chess.bitwiseandrea.com/super-chess.json`) and enable
   HttpService — the server will then re-fetch the latest JSON on boot and
   silently fall back to the embedded snapshot if the request fails.

The Luau-only `implemented` flag is **not** part of the shared data: which card
effects are wired up in Roblox is derived at runtime from
`CardEffects.HANDLERS`, so adding/removing implementations doesn't require
re-editing `super-chess.json`.
...
ReplicatedStorage/
  SuperChess/
    Modules/
      ...
      Cards            ModuleScript -- card lookup (reads from SharedData)
      Deck             ModuleScript -- draw/discard/hand mgmt (max hand size from SharedData)
      CardEffects      ModuleScript -- effect handlers
      PieceVisuals     ModuleScript -- graybox piece factory (cylinders + blocks)
      Bot              ModuleScript -- alpha-beta minimax bot (PSTs + values from SharedData)
      SuperChessData   ModuleScript -- GENERATED: embedded snapshot of public/super-chess.json
      SharedData       ModuleScript -- canonical accessor + optional HttpService boot fetch
    Remotes/
...
| Capture-triggered card draw | yes | yes | white still skips first draw to offset first-move advantage; this is the only card-draw trigger |
